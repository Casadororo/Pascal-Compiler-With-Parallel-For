...
void TabelaSimbolos::coloca_tipo_em_simbolos(Tipo *tipo, int quantidade) {
...
void TabelaSimbolos::coloca_tipo_em_simbolos(tipo_parametro_variavel tipo,
                                             int quantidade) {
  if (this->is_stack_empty()) {
    std::cerr << "A tabela esta vazia\n";
    return;
...
void TabelaSimbolos::coloca_tipo_em_simbolos(tipo_parametro tipo,
                                             int quantidade) {
  if (this->is_stack_empty()) {
    std::cerr << "A tabela esta vazia\n";
    return;
...
void TabelaSimbolos::coloca_tipo_em_simbolos(Tipo *tipo, int vector_size,
                                             int quantidade) {
  if (this->is_stack_empty()) {
    std::cerr << "A tabela esta vazia\n";
    return;
  ...
  while (current != nullptr && count < quantidade) {
    current->simbolo->tipo_v = tipo;
    current->simbolo->vector_size = vector_size;
    current = current->prev;
    count++;
  }
...
void print_simbolo(Simbolo *s) {
  if (s != nullptr) {
    printf("Simbolo: %s, Tipo: %s, Nivel Lexico: %d, Deslocamento: %d\n",
           s->identificador.c_str(), s->tipo_v->identificador.c_str(),
           s->nivel_lexico, s->deslocamento);
  } else {
...
void TabelaSimbolos::print_var_simbolo(Simbolo *simbolo) {
  printf("│ %-11s │ %-12s │ %-12s │ %-13d │ %-18d │ %-13s │ %-14s │ %-14s │ %-14d │\n",
         simbolo->identificador.c_str(), "Variavel",
         simbolo->tipo_v->identificador.c_str(), simbolo->nivel_lexico,
...
void TabelaSimbolos::print_process_simbolo(Simbolo *simbolo) {
  printf("│ %-11s │ %-12s │ %-12s │ %-13d │ %-18s │ %-13s │ %-14s │ %-14s │\n",
         ...
         (simbolo->tipo_simbo == function ? simbolo->identificador.c_str()
                                          : "N/A"),
         simbolo->nivel_lexico,
         (simbolo->tipo_simbo == function
              ? std::to_string(simbolo->deslocamento).c_str()
              : "N/A"),
         "N/A", // Tipo Param Var não aplicável
         (simbolo->tipo_simbo == function
              ? (simbolo->tipo_param == t_copy ? "Copia" : "Ponteiro")
              : "N/A"),
         simbolo->rotulo_enpr() != nullptr
             ? simbolo->rotulo_enpr()->identificador.c_str()
             : "N/A");
}
...
void TabelaSimbolos::print_tabela() {
  ...
  printf("├─────────────┬──────────────┬──────────────┬───────────────┬────────────────────┬───────────────┬────────────────┬────────────────┬────────────────┤\n");
  printf("│   Símbolo   │ Tipo Simbolo │     Tipo     │ Nível Léxico  │   Deslocamento     │ Tipo Param Var│ Tipo Parametro │     Rótulo     │  Tamanho Vetor │\n");
  printf("├─────────────┼──────────────┼──────────────┼───────────────┼────────────────────┼───────────────┼────────────────┼────────────────┼────────────────┤\n");
